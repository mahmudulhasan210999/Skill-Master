        ...
        <div class="flex flex-col items-center px-6 py-8 md:px-40 md:py-12 bg-green-50">
            <div class="container">
                <div class="md:flex items-center">
                    <div class="px-4">
                        <img class="rounded-full h-40 w-40" :src="instructor.profile_photo" alt="Image">
                    </div>
                    <div class="pl-8 pt-4 md:pt-0 md:pl-4">
                        <div class="flex text-left">
                            <p class="text-xs text-red-600 px-2 py-1 bg-rose-200 rounded-sm text-left">{{ instructor.experience }} Year Exp.</p>
                        </div>

                        <p class="text-xl font-semibold py-1">{{ instructor.name }}</p>
            ...
            <div class="container">
                ...
                <div v-if="educationDetails">
                    <div class="px-6">
                        ...
                        <div v-for="(edu, index) in instructor.education" :key="index">
                            <div class="flex">
                                <div>
                                    <p class="text-primary md:px-4 py-2">★</p>
                                </div>
                                <div class="text-sm py-2">
                                    <p class="text-primary font-semibold p-1">{{ edu.degree }}</p>
                                    <div class="text-slate-600">
                                        <p class=" p-1">{{ edu.passing_year }}</p>
                                        <p class=" p-1">{{ edu.university }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    computed: {
        ...mapState ({ 
            instructor: state => state.instructors.instructor
        }),
    },

    mounted() {
        this.$store.dispatch('instructors/getInstructorByCourseId', this.i_id )
    }
}
</script>
