    ...
    <div class="w-full flex flex-col items-center px-4 md:px-8 lg:px-12 xl:px-28 pb-16">
        <div class="container">
            <div class="text-center p-8 sm:p-12">
                <p class="text-2xl md:text-3xl lg:text-4xl font-bold px-4 pb-4">New Top Articles</p>
            ...
            <div class="product-list">
                <div v-for="(blog, index) in blogs" :key="index">
                    <div>
                        <div>
                            <router-link :to="{ path: '/blog/' + blog.slug}"><img class="rounded-t-lg lg:p-0 h-48 sm:h-56 xl:h-60 w-full small" :src="blog.headerImage" alt="Image"></router-link>
                        </div>
...
export default {
    ...
    computed: {
        ...mapState ({
            blog: state => state.blogs.blog_by_category, 
            count: state => state.blogs.blog_count,
        }),
    },
    ...
    mounted() {
        this.$store.dispatch('blogs/getBlogsByCategory', {slug: this.$route.params.id, current: this.current})
    },
...
<style scoped>
    .product-list {
        @apply grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 sm:px-8;
    }
    ...
    @media screen and (max-width: 490px) {
        .product-list {
            @apply px-8 grid grid-cols-1 gap-6;
        }
